...
const Confirmation = (props) => {
  ...
  const sendDtrf = async () => {
    ...
    const response = await reqWithToken(url)
  ...
  const handleOnClickBDM = async () => {
    ...
    const response = await reqWithToken(url, "POST", data)
    if (response.status == 200) {
      successMessage("Submitted to BDM succesfully")
      Router.push("/super-dtrf")
    } else {
  ...
  const postData = async (values, fromButton) => {
    ...
    let response
    response = await reqWithToken(url, "POST", formData, "multi")
