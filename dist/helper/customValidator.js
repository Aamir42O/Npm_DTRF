...
exports.validateField = exports.cytoFlow = exports.niptFlow = exports.nbsFlow = exports.pnsField = void 0;
...
exports.cytoFlow = cytoFlow;

const validateField = (field, formValues, externalValidation, setValues) => {
  const errors = {};
  console.log(externalValidation);

  if (externalValidation.calculateCurrentGestational) {
    console.log("condition", formValues.usgDate && formValues.sampleCollectionDate && (0, _commonHelper.hasValue)(formValues.gestationalAgeDays) && (0, _commonHelper.hasValue)(formValues.gestationalAgeWeeks));

    if (formValues.usgDate && formValues.sampleCollectionDate && (0, _commonHelper.hasValue)(formValues.gestationalAgeDays) && (0, _commonHelper.hasValue)(formValues.gestationalAgeWeeks)) {
    ...
    if ((0, _commonHelper.hasValue)(formValues.currentGestationalAgeDays) && (0, _commonHelper.hasValue)(formValues.currentGestationalAgeWeeks) && externalValidation.validateGestation) {
  ...
  for (let i = 0; i < field.length; i++) {
    ...
    if (validation.includes("atLeastOne") && (0, _commonHelper.hasValue)(value)) {
      let isErr = true;
    ...
    if (validation.includes("minLength")) {
      if (value.length < value && (0, _commonHelper.hasValue)(value)) {
    ...
    if (validation.includes("isFutureDate")) {
      ...
      if (diff >= days && (0, _commonHelper.hasValue)(value)) {
        errors[field[i].name] = "Date should be future ".concat(days > 0 ? "or current" : "", " date");
        continue;
      }
    }
    ...
    if (validation.includes("isPastDate")) {
      ...
      if (diff < days && (0, _commonHelper.hasValue)(value)) {
        errors[field[i].name] = "Date cant be future ".concat(days > 0 ? "or current" : "", " date");
        continue;
      }
    }

    if (validation.includes("minValue")) {
      if (type == "date" && (0, _commonHelper.hasValue)(value)) {
        const diff = moment(moment().format("YYYY-MM-DD")).diff(formValues[field[i].name], minValue.type);

        if (Math.abs(diff) < minValue.value) {
          errors[field[i].name] = "Date should be ".concat(minValue.value < 0 ? "after" : "before", " or equal to   ").concat(Math.abs(minValue.value), "  ").concat(minValue.type, " from   current date");
          continue;
        }
      } else if (value < field[i].validationValue.minValue && (0, _commonHelper.hasValue)(value)) {
    ...
    if (validation.includes("maxValue")) {
      if (type == "date" && (0, _commonHelper.hasValue)(value)) {
        ...
        if (Math.abs(diff) > maxValue.value) {
          errors[field[i].name] = "Date should be ".concat(maxValue.value < 0 ? "after" : "before", " or equal to   ").concat(Math.abs(maxValue.value), "  ").concat(maxValue.type, " from   current date");
          continue;
        }
      } else if (value > field[i].validationValue.maxValue && (0, _commonHelper.hasValue)(value)) {
    ...
    if (validation.includes("greaterThanEqual")) {
      if (type == "date" && (0, _commonHelper.hasValue)(value)) {
        ...
        if (data && value < data && (0, _commonHelper.hasValue)(value)) {
    ...
    if (validation.includes("lessThanEqual")) {
      ...
      } else if (data && value > data && (0, _commonHelper.hasValue)(value)) {
